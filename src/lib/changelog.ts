...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: '2025-11-26',
    added: [
      '📝 新增《OpenWrt 部署后续指导》文档，整理常见部署/排障问答',
      '📄 发布 `docs/release-v0.7.0.md`，便于快速同步版本亮点',
    ],
    changed: [
      '🎨 移动端注册页重新排版，验证码区域自适应小屏并支持一键刷新',
      '⚙️ 客户端版本检测改为调用 `/api/version/check`，避免浏览器缓存与跨域问题',
      '🧠 搜索排名仅允许按顺序的子序列匹配，降低“ABC 匹配 ACB”误命中',
      '🌐 版本检测后端支持自定义仓库/分支并加入多镜像源，保障小更新仅靠时间戳即可提示',
    ],
    fixed: [
      '🐛 搜索结果中带查询参数的 `.m3u8` 链接不再被误判为不可播放',
      '🐛 远程版本时间戳获取失败时不再误报“已是最新”，会正确提示无法检查更新',
    ],
  },
  {
    version: '0.6.0',
